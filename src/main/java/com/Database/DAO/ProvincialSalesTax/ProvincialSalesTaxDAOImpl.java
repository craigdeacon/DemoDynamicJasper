...
public class ProvincialSalesTaxDAOImpl implements ProvincialSalesTaxDAO
{
    ...
    public ArrayList<ProvincialGroup> getGroupsProvincialSalesTax()
    {
        ...
        String SQL_GET_PROVINCIAL_GROUPS = "SELECT\n"
                + "	settings.strval,\n"
                + "	settings.var_id,\n"
                + "	pst_report.month_id,\n"
                + "	pst_report.group_id,\n"
    ...
    {
        HashMap<String, Integer> underwritersMap = new HashMap<>();
        String SQL_GET_UNDERWRITERS = "SELECT\n"
                + "    DISTINCT invoice_detail5.underwriter_id,\n"
                + "    underwriter.under_name\n"
                + "FROM\n"
                + "    invoice_detail5\n"
                + "JOIN invoice ON\n"
                + "    invoice.month_id = invoice_detail5.month_id\n"
                + "    AND invoice.group_id = invoice_detail5.group_id\n"
                + "JOIN underwriter ON\n"
                + "    underwriter.underwriter_id = invoice_detail5.underwriter_id\n"
                + "WHERE\n"
                + "    invoice_detail5.month_id = 'December, 1, 2018'\n"
                + "GROUP BY\n"
                + "    invoice_detail5.underwriter_id,\n"
                + "    underwriter.under_name";
        jdbcTemplate.query(SQL_GET_UNDERWRITERS,
                resultSet ->
        {
            underwritersMap.put( resultSet.getString( "under_name"), resultSet.getInt( "underwriter_id" ) );
        } );
        return underwritersMap;
    }
