...
public class IAPPremiumReport
{
    ...
    public void displayIAPPremiumReport()
    {
        try
        {
            ...
            ArrayList<AbstractColumn> columnList = getIAPPremiumColumns();
            ...
            JRDataSource dataSource = new JRBeanCollectionDataSource(iapPremiumReportBO.getIapPremiumGroups() );

            JasperPrint jasperPrint = DynamicJasperHelper.generateJasperPrint(dynamicReport, new ClassicLayoutManager(), dataSource );

            ReportUtilities.exportExcel(jasperPrint, "IAP Premium Report", false);
        }
        catch (JRException ex)
        {
            Logger.getLogger(IAPPremiumReport.class.getName() ).log(Level.SEVERE, "displayIAPPremiumReport Failed", ex );
        }
    }

    private ArrayList<AbstractColumn> getIAPPremiumColumns()
    {
        ...
        AbstractColumn benefitAmountPerEmployee = createColumnString("benefitAmountPerEmployee", "Benefit Amount Per Employee", 100);
        ...
        AbstractColumn benefitAmountDependent = createColumnString("benefitAmountDependent", "Benefit Amount (Dependent)", 100);
